...
public class TurnDegreesTimeOutCommand implements AutonCommand {
    ...
    public TurnDegreesTimeOutCommand(double angle, 
        ...
        if (Math.abs (angleGoal) > 150 && vacjume.getVacjumePot() > 450) {
        gyroPID = new PIDController(Constants.getDouble("turnP180"),
                                    Constants.getDouble("turnI180"),
                                    Constants.getDouble("turnD180")); 
        }
        
        else if (Math.abs (angleGoal) > 150 && vacjume.getVacjumePot() < 450) {
            gyroPID = new PIDController(Constants.getDouble("turnPVacDown"), Constants.getDouble("turnIVacDown"), Constants.getDouble("turnDVacDown"));
        }
